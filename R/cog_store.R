...
#'
#' Folding the encoding in makes the key identify the BYTES, so any encoding
#' change publishes to a NEW url and no cache can be stale. Dedup is unaffected:
#' identical payloads written the same way still collapse to one object.
#'
#' @param content_hash payload hash(es) from [content_hashes()]
#' @param enc short encoding tag, e.g. `"flt4s-nd9999-noovr"`
#' @return 16-char hex hash(es) identifying payload + encoding
#' @importFrom digest digest
#' @export
#' @concept cog_store
content_hash_encoded <- function(content_hash, enc) {
  stopifnot(is.character(enc), length(enc) == 1L, nzchar(enc))
  vapply(content_hash, function(h)
    substr(digest::digest(paste(h, enc, sep = "|"), algo = "xxhash64"), 1, 16),
