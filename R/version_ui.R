...
#'   (default `?ver={v}`)
#' @param href_restricted link builder for restricted versions, or `NULL` to use
#'   `href` (the preview instance, where restricted versions render in place).
#'   The public app passes e.g.
#'   `function(v) sprintf("%s/scores/?ver=%s", atlas_preview_url(), v)`.
...
version_picker_html <- function(current, versions = NULL,
  ...
  # a hand-built data frame (tests, docs) may predate the column: same
  # fail-closed default as atlas_versions()
  if (!"access" %in% names(versions)) versions$access <- atlas_access_default(versions$status)

  badge <- function(status, access) paste0(switch(
    status,
    released   = "",
    ...
    if (identical(access, "restricted"))
      " <span class='badge bg-dark' title='under review \u2014 sign-in required'>\U0001F512 restricted</span>"
    else "")
  link <- function(v, access)
    if (identical(access, "restricted") && !is.null(href_restricted)) href_restricted(v) else href(v)
  ...
  rows <- vapply(seq_len(nrow(versions)), function(i) {
    ...
    if (identical(v, current))
      sprintf(paste0("<li class='list-group-item active d-flex justify-content-between",
              ...
              v, badge(versions$status[i], versions$access[i]), ttl, dt)
    else
      sprintf(paste0("<li class='list-group-item d-flex justify-content-between",
              ...
              link(v, versions$access[i]), v, badge(versions$status[i], versions$access[i]), ttl, dt)
  }, character(1))
