...
#       20 areas and >= 49.1447 on GEO (turtle, 1.41 % coverage). If it ever stops
...
# (d) the measured turtle margin on GEO is 49.14477253; the floor is stated one
# digit shy of it, because the assertion is "still this badly wrong", not "wrong to
# the last ulp" -- DuckDB's parallel SUM reorders float addition between runs and
# the blend deltas themselves jitter at 1e-13.
GEO_MIN   <- 49.1447
N_OLD_MIN <- 12        # (d) how many areas the old formula must still get wrong
...
if (is.null(fx)) {
  fail <- c(fail, "(c) the programarea_gaa fixture is missing")
} else {
  traced <- nrow(cells_in_polygon_grid(fx$geometry, fx$grid))
  want   <- tab$n_cells[tab$pra == "GAA"]
  if (!length(want)) want <- NA_integer_
  if (!isTRUE(traced == want))
    fail <- c(fail, sprintf(
      "(c) the traced GAA outline gives %d cells, the published zone_cell %s",
      traced, format(want)))
  say(sprintf("(c) GAA traced from its outline: %d cells, published zone_cell %s  %s",
              traced, format(want), if (isTRUE(traced == want)) "OK" else "FAIL"))
}
...
if (!isTRUE(geo >= GEO_MIN))
  fail <- c(fail, sprintf(
    "(d) the OLD formula no longer fails on GEO: %s < %.4f. Either the data changed ",
    format(round(geo, 4)), GEO_MIN))
if (n_bad < N_OLD_MIN)
  fail <- c(fail, sprintf(
    "(d) the OLD formula exceeds %.1f on only %d of %d areas, expected >= %d",
    TOL_CLIP, n_bad, nrow(tab), N_OLD_MIN))
say(sprintf("(d) the OLD formula still FAILS: GEO %s >= %.4f, and %d of %d areas exceed %.1f  %s",
            format(round(geo, 4)), GEO_MIN, n_bad, nrow(tab), TOL_CLIP,
            if (isTRUE(geo >= GEO_MIN) && n_bad >= N_OLD_MIN) "OK" else "FAIL"))
say("    (on GAA alone the old formula misses by only ",
    sprintf("%.4f", tab$d_old[tab$pra == "GAA"]),
